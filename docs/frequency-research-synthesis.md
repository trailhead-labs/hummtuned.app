...
Two claims get blurred together here, and only one is solid (framing imported from the public /science page research pass, 2026-07-06). **The neural response is real and measurable:** the brain produces a matching auditory steady-state response to the beat, traced along the pathway from brainstem to cortex (**eNeuro 2020**). That the brain registers the beat, specifically, is not in dispute. **Whether that response drives the outcome is not established** — the mechanism the whole feature rests on, that the beat "entrains" the brain and shifts your state with it, is unproven. The best current synthesis, **Ingendoh, Posny & Heine 2023 (PLOS ONE, 14 EEG studies)**, found 5 supporting entrainment, 8 contradicting, 1 mixed, and concluded it "cannot be settled." Notably **no study demonstrated beta-band entrainment**, and every study embedding the beat in pink noise found none. The one meta-analysis with a real aggregate effect (**Garcia-Argibay 2019**, g ≈ 0.45) is driven mostly by **anxiety/relaxation** (theta/delta), not by arousal or cognition.
